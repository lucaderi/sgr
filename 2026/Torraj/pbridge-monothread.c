...
void print_stats() {
  ...
  fprintf(stderr, "\n=========================================\n");
  fprintf(stderr, "BRIDGE TOTAL STATS (Uptime: %.2f s)\n", deltaSec);
  fprintf(stderr, "Total: %llu pkts [%.1f pkt/s] - %llu bytes [%.2f Mbit/s]\n",
          numPkts, (double)numPkts/deltaSec, numBytes, (double)8*numBytes/(double)(deltaSec*1000000));
  
  if(pd && pcap_stats(pd, &ps) >= 0)
    fprintf(stderr, "IF 1: Rcv=%u, Drop=%u\n", ps.ps_recv, ps.ps_drop);
  if(pd2 && pcap_stats(pd2, &ps) >= 0)
    fprintf(stderr, "IF 2: Rcv=%u, Drop=%u\n", ps.ps_recv, ps.ps_drop);
  fprintf(stderr, "=========================================\n");
}
...
void dummyProcesssPacket(u_char *_deviceId, const struct pcap_pkthdr *h, const u_char *p) {
    struct bridge_args *a = (struct bridge_args *)_deviceId;
    pcap_t *out = a->out;
    ...
    if(verbose) {
        ...
        printf("%02d:%02d:%02d.%06u ", s / 3600, (s % 3600) / 60, s % 60, (unsigned)h->ts.tv_usec);
        memcpy(&ehdr, p, sizeof(struct ether_header));
        eth_type = ntohs(ehdr.ether_type);
        printf("[%s -> %s] ", etheraddr_string(ehdr.ether_shost, b1), etheraddr_string(ehdr.ether_dhost, b2));
        ...
        if(eth_type == 0x0800) {
            ...
            printf("[%s][%s -> %s] ", proto2str(ip.ip_p), intoa(ntohl(ip.ip_src.s_addr)), intoa(ntohl(ip.ip_dst.s_addr)));
        ...
        else printf("[eth_type=0x%04X]", eth_type);

        printf("[caplen=%u][len=%u]\n", h->caplen, h->len);
    }

    //Modifica: reindirizzo il pacchetto all'altro device
    if (pcap_inject(out, p, h->caplen) == -1)
        fprintf(stderr, "[%s] Errore inject: %s\n", a->name, pcap_geterr(out));

    if(a->pkts == 0 && startTime.tv_sec == 0) gettimeofday(&startTime, NULL);
    a->pkts++;
    a->bytes += h->len;
...
void printHelp(void) {
  ...
  printf("Usage: pbridge-monothread [-h] -i <dev1> -o <dev2> [-v <1|2>] [-f <filter>]\n");
...
int main(int argc, char* argv[]) {
  char *device1 = NULL, *device2 = NULL, *bpfFilter = NULL;
  u_char c;
  ...
  if(bpfFilter != NULL) {
    ...
    if(pcap_compile(pd2, &fcode, bpfFilter, 1, 0xFFFFFF00) < 0) {
      printf("pcap_compile error: '%s'\n", pcap_geterr(pd2));
    } else {
  ...
  // if(bpfFilter) {
  //   pcap_compile(pd, &fcode, bpfFilter, 1, 0xFFFFFF00); pcap_setfilter(pd, &fcode);
  //   pcap_compile(pd2, &fcode, bpfFilter, 1, 0xFFFFFF00); pcap_setfilter(pd2, &fcode);
  // }
  ...
  //Preparo gli argomenti per dummtProcessPacket
  struct bridge_args args1to2 = { pd, pd2, "", 0, 0 };
  struct bridge_args args2to1 = { pd2, pd, "", 0, 0 };
  snprintf(args1to2.name, 16, "%s->%s", device1, device2);
  snprintf(args2to1.name, 16, "%s->%s", device2, device1);

  printf("Bridge Monothread attivo: %s <--> %s\n", device1, device2);
    ...
    if (select(max_fd + 1, &read_fds, NULL, NULL, NULL) > 0) {
      ...
      if (FD_ISSET(fd1, &read_fds)) {
        if (pcap_next_ex(pd, &header, &pkt_data) > 0)
          dummyProcesssPacket((u_char*)&args1to2, header, pkt_data);
      }
      if (FD_ISSET(fd2, &read_fds)) {
        if (pcap_next_ex(pd2, &header, &pkt_data) > 0)
          dummyProcesssPacket((u_char*)&args2to1, header, pkt_data);
      }
    }
  }

  print_stats();
  pcap_close(pd); pcap_close(pd2);
