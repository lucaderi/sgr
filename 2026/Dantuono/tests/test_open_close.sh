...
PORT="${EVENT_TEST_PORT:-18082}"
REGEX="^netmon_service_up\\{[^}]*protocol=\"tcp\"[^}]*port=\"$PORT\"[^}]*\\} 1$"

OPEN_BEFORE="$(metric_value netmon_open_events_total)"
CLOSE_BEFORE="$(metric_value netmon_close_events_total)"
OPEN_BEFORE="${OPEN_BEFORE:-0}"
CLOSE_BEFORE="${CLOSE_BEFORE:-0}"

python3 -m http.server "$PORT" --bind 0.0.0.0 >/dev/null 2>&1 &
SERVICE_PID=$!

if ! wait_for_present "$REGEX"; then
  echo "[FAIL] Il servizio di test non è comparso nelle metriche." >&2
  cat "$METRICS_FILE" >&2
  exit 1
fi

OPEN_AFTER="$(metric_value netmon_open_events_total)"
OPEN_AFTER="${OPEN_AFTER:-0}"

if (( OPEN_AFTER <= OPEN_BEFORE )); then
  echo "[FAIL] Il contatore OPEN non è aumentato: $OPEN_BEFORE -> $OPEN_AFTER" >&2
  exit 1
fi
...
if ! wait_for_absent "$REGEX"; then
  echo "[FAIL] Il servizio di test non è scomparso dalle metriche." >&2
  cat "$METRICS_FILE" >&2
  exit 1
fi

CLOSE_AFTER="$(metric_value netmon_close_events_total)"
CLOSE_AFTER="${CLOSE_AFTER:-0}"

if (( CLOSE_AFTER <= CLOSE_BEFORE )); then
  echo "[FAIL] Il contatore CLOSE non è aumentato: $CLOSE_BEFORE -> $CLOSE_AFTER" >&2
  exit 1
fi

echo "[PASS] OPEN incrementato: $OPEN_BEFORE -> $OPEN_AFTER"
echo "[PASS] CLOSE incrementato: $CLOSE_BEFORE -> $CLOSE_AFTER"
