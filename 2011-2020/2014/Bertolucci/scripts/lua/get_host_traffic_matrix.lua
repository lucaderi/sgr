...
if(host == nil) then
	...
	for key, values in pairs(hosts_stats) do --[[key ritorna ip computer]]--
	   ...
	   if(host ~= nil) then
	      ...
	      if(host["contacts"]["client"] ~= nil) then 
			 for k,v in pairs(host["contacts"]["client"]) do 
			    ...
			    print('\n{"source":'..key_id..',"source_ip":"'..host["ip"]..'","target":'..peer_id..',"target_ip":"'..k..'","value":'..v..',"styleColumn":"client"}')
			    links = links + 1
			 end
	      end
	      ...
	      if(host["contacts"]["server"] ~= nil) then
			for k,v in pairs(host["contacts"]["server"]) do 
			    ...
			    hosts_id[key]['count'] = hosts_id[key]['count'] + v
			    if(links > 0) then print(",") end
			    print('\n{"source":'..key_id..',"source_ip":"'..host["ip"]..'","target":'..peer_id..',"target_ip":"'..k..'","value":'..v..',"styleColumn":"server"}')
			    links = links + 1
		 	end
	      end
	   end
