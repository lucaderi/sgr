...
public class ServersAnalysis extends HttpServlet {
	...
	if (filter==0) {out.println("Invalid Request."); System.exit(0);}
	    ...
	    String sql = "SELECT server, SUM(bytes) AS nbytes, SUM(time) AS time, AVG(slatency) AS slatency FROM pages WHERE timestamp>'"+df.format(calendar.getTime())+"'GROUP BY 1 ORDER BY 2 DESC";
		...
		out.println("<table border=\"10px\">"+
			...
			"<td width=\"75px\" align=\"center\"><b>Data Sent</td>"+
		...
		while(rs.next()){
		    ...
		    out.println("<tr><td width=\"275px\" ><A HREF=\"index.jsp?mod=sdetails&server="+rs.getString("server")+"&StartTime="+req.getParameter("StartTime")+"\">"+rs.getString("server")+"</A>"+
				...
				"</td><td width=\"75px\" align=\"center\">"+trasformTime(rs.getLong("time"))+
