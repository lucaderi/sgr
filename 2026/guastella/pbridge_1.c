...
void dummyProcesssPacket(u_char *_deviceId,
  ...
  if(pd_out != NULL){
    if(pcap_sendpacket(pd_out, p, h->caplen) != 0){
...
void printHelp(void) {
  ...
  printf("-o <out_device>  [Output device for bridging]\n");
...
int main(int argc, char* argv[]) {
  ...
  /*Opening output interface (if present) */
  if(out_device != NULL) {
    printf("Bridging to output device %s\n", out_device);
    promisc = 1;
    if((pd_out = pcap_open_live(out_device, snaplen, promisc, 500, errbuf)) == NULL) {
      printf("pcap_open_live (out_device): %s\n", errbuf);
      return(-1);
    }
  }
