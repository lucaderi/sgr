...
void ndpi_search_modbus_tcp(struct ndpi_detection_module_struct *ndpi_struct,
    ...
    u_int16_t modbus_port = htons(502); // porta used by modbus
    /* Check connection over TCP */
    
    if(packet->tcp) {
    
   if((packet->payload_packet_len >= 8) // the payload of Modbus-TCP segment must be at least 8 bytes (7 bytes of header application packet plus 1 byte of minimum payload of application packet)

     &&((packet->tcp->dest == modbus_port) || (packet->tcp->source == modbus_port))){ // Modbus uses the port 502
		
            u_int16_t modbus_len = htons(*((u_int16_t*)&packet->payload[4])); // the fourth parameter of the payload is the length of the segment
