...
/* Struttura per passare i due handle al thread */
struct bridge_args {
    pcap_t *in;    /* interfaccia di ingresso */
    pcap_t *out;   /* interfaccia di uscita   */
    char name[16]; /* nome del thread per i log */
    unsigned long long pkts;   /* contatore pacchetti di questo thread */
    unsigned long long bytes;  /* contatore byte di questo thread      */
};
...
if(dumper)
  ...
  if(a->pkts == 0) gettimeofday(&startTime, NULL);
  a->pkts++;
  a->bytes += h->len;
...
void* thread_bridge(void *arg) {
    ...
    printf("Thread %s avviato\n", a->name);
    /* pcap_loop si blocca qui, ma solo per questo thread —
       l'altro thread continua a girare tranquillamente */
...
int main(int argc, char* argv[]) {
  ...
  if(stat(device2, &s) == 0) {
    /* Device is a file on filesystem */
    if((pd2 = pcap_open_offline(device2, errbuf)) == NULL) {
      printf("pcap_open_offline: %s\n", errbuf);
      return(-1);
    }
  } else {
    /* hardcode: promisc=1, to_ms=500 */
    promisc = 1;
    if((pd2 = pcap_open_live(device2, snaplen, promisc, 500, errbuf)) == NULL) {
      printf("pcap_open_live: %s\n", errbuf);
      return(-1);
    }
  }
  ...
  /* Prepara gli argomenti per i due thread */
    struct bridge_args args0 = { pd,  pd2, "", 0, 0 };
    struct bridge_args args1 = { pd2, pd,  "", 0, 0 };
    snprintf(args0.name, sizeof(args0.name), "%s->%s", device1, device2);
    snprintf(args1.name, sizeof(args1.name), "%s->%s", device2, device1);

/* Crea i due thread */
    pthread_t t0, t1;
    pthread_create(&t0, NULL, thread_bridge, &args0);
    pthread_create(&t1, NULL, thread_bridge, &args1);

    printf("Bridge bidirezionale attivo: %s <--> %s\n", device1, device2);
    printf("Premi Ctrl+C per fermare.\n");

  /* Aspetta che entrambi i thread finiscano (dopo Ctrl+C) */
    pthread_join(t0, NULL);
    pthread_join(t1, NULL);

    /* Statistiche per thread */
    fprintf(stderr, "=========================\n");
    fprintf(stderr, "[%s] %llu pkts, %llu bytes\n",
            args0.name, args0.pkts, args0.bytes);
    fprintf(stderr, "[%s] %llu pkts, %llu bytes\n",
            args1.name, args1.pkts, args1.bytes);
    fprintf(stderr, "=========================\n");
