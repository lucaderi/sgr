...
void printTrafficSummary (int revertOrder) {
  ...
  if(myGlobals.pcap_file_list == NULL) {
    safe_snprintf(__FILE__, __LINE__, buf, sizeof(buf), "<TR "TR_ON"><TH "TH_BG" ALIGN=LEFT "DARK_BG">Capturing Since</TH>"
...
static int cmpTalkersFctn(const void *_a, const void *_b) {
...
void printMulticastStats(int sortedColumn /* ignored so far */,
  ...
  sendString("<P ALIGN=right><A class=external "
	     "HREF=http://www.iana.org/assignments/multicast-addresses>List</A> "
	     "of multicast addresses</P>\n");
}
...
void printAllSessionsHTML(char* host, int actualDeviceId, int sortedColumn,
  ...
  sendString("\n\n<div id=\"tabs-0\">\n");
  {
	...
	if(token) {
	  token[0] = '\0';
	  sendString(line);
	  (el->hostResolvedName[0] != '\0') ? sendString(el->hostResolvedName) : sendString(host);
	  sendString(&token[6]);
	} else
	  sendString(line);
      }
...
void printNoDataYet(void) {
  printFlagedWarning(CONST_NO_DATA_YET);
}
...
void printHostHourlyTraffic(HostTraffic *el) {
  ...
  if(tcSent > 0) {
    ...
    hostReport(3, targetStr, el->vlanId, 1);
    sendString("</TD>");
  } else
  ...
  if(tcRcvd > 0) {
    sendString("<TD "TD_BG" ALIGN=RIGHT COLSPAN=2 BGCOLOR=white>");
    hostReport(3, targetStr, el->vlanId, 0);
    sendString("</TD>");
  } else
    sendString("<TD COLSPAN=2 "TD_BG">&nbsp;</TD>\n");
