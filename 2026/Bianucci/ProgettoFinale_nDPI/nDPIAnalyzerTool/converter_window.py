#!/usr/bin/env python3
""" Modulo per la conversione da file PCAP/PCAPNG a TXT tramite ndpiReader """
...
class PcapConverterWindow(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="Estrai Metadati con nDPI", font=("Arial", 14, "bold"), bg=COLORS["panel"], fg=COLORS["text"]).pack(anchor="w")
        tk.Label(header, text="Esegue ndpiReader -v 2 sul file di cattura e produce il dump per l'assistente.", font=("Arial", 12), bg=COLORS["panel"], fg=COLORS["muted"]).pack(anchor="w")

        # Selezione PCAP
        f_pcap = tk.Frame(self, bg=COLORS["panel"])
        f_pcap.pack(fill="x", **pad)
        tk.Label(f_pcap, text="File di cattura (.pcap / .pcapng):", bg=COLORS["panel"], fg=COLORS["text"], font=("Arial", 10, "bold")).pack(anchor="w")
        self.ent_pcap = self._mostra_pulsante("pcap", f_pcap)

        # Destinazione TXT
        f_txt = tk.Frame(self, bg=COLORS["panel"])
        f_txt.pack(fill="x", **pad)
        tk.Label(f_txt, text="Salva file di output (.txt):", bg=COLORS["panel"], fg=COLORS["text"], font=("Arial", 10, "bold")).pack(anchor="w")
        self.ent_txt = self._mostra_pulsante("txt", f_txt)

        # Status
        self.lbl_status = tk.Label(self, text="", bg=COLORS["panel"], fg=COLORS["success"], font=("Arial", 10))
        self.lbl_status.pack(fill="x", padx=20, pady=(6, 0))

        # Pulsanti inferiori
        f_btn = tk.Frame(self, bg=COLORS["panel"])
        ...
        self.btn_use = tk.Button(f_btn, text="Usa nel Chatbot ➔", bg=COLORS["success"], fg=COLORS["bg"], font=("Arial", 11, "bold"), relief="flat", state="disabled", command=self._use_in_chat)
    ...
    def _mostra_pulsante(self, tipo: str, f_file: tk.Frame) -> tk.Entry:
        row_file = tk.Frame(f_file, bg=COLORS["panel"])
        row_file.pack(fill="x", pady=4)
        ent_file = tk.Entry(row_file, bg=COLORS["panel_alt"], fg=COLORS["text"], insertbackground="white", relief="flat", font=("Arial", 10)) # casella di testo che contiene il path del file
    ...
    def _start_conversion(self):
        ...
        self.lbl_status.configure(text="Esecuzione di ndpiReader -v 2 in corso...", fg=COLORS["accent"])
    ...
    def _on_success(self, txt_path):
        ...
        self.lbl_status.configure(text="Estrazione completata con successo!", fg=COLORS["success"])
    ...
    def _on_error(self, err_msg):
        self.btn_run.configure(state="normal", text="⚡  Avvia Estrazione")
        self.lbl_status.configure(text="Errore durante l'estrazione", fg=COLORS["danger"])
