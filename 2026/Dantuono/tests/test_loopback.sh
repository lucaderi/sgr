...
if grep -Eq "$REGEX" "$METRICS_FILE"; then
  echo "[FAIL] Il listener loopback 127.0.0.1:$PORT è stato esportato, ma dovrebbe essere ignorato." >&2
  cat "$METRICS_FILE" >&2
  exit 1
else
  echo "[PASS] Listener loopback 127.0.0.1:$PORT correttamente ignorato."
fi
