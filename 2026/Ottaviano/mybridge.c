...
char* format_numbers(double val, char *buf, unsigned int buf_len, u_int8_t add_decimals)	{
  unsigned int a1 = ((unsigned long)val / 1000000000) % 1000;	
  unsigned int a = ((unsigned long)val / 1000000) % 1000;
  unsigned int b = ((unsigned long)val / 1000) % 1000;
  unsigned int c = (unsigned long)val % 1000;
  unsigned int d = (unsigned int)((val - (unsigned long)val)*100) % 100;
...
void dummyProcesssPacket(unsigned  *_deviceId,
			 const struct pcap_pkthdr *h,
